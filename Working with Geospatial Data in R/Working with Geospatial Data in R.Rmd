...
Ref: Wickham, Charlotte. https://www.datacamp.com/courses/working-with-geospatial-data-in-r, 2018.
...
* CRS: a coordinate reference system. Common CRS: longitude, latitude. 

* Point data: locations are points, described by single pair of coordinates

How to grab a background map: 

* step 1: download a map using: *get_map()*

* step 2: display the map using *ggmap()*

*get_map()* has a number of arguments that control what kind of map to get. 


## 1.1 get_map and ggmap()
...
# add sales data
dir("data/")
sales <- read_rds("data/01_corv_sales.rds")
glimpse(sales)
head(sales)

# map color to year_built
ggmap(corvallis_map) +
  geom_point(aes(x = lon, y = lat, color = year_built), data = sales)

# map size to bedrooms 

ggmap(corvallis_map) + 
    geom_point(aes(x = lon, y = lat, size = bedrooms), data = sales, alpha = 0.5)

# map color to price/finished_squaredfeet

ggmap(corvallis_map) +
  geom_point(aes(x = lon, y = lat, color = price/finished_squarefeet), data = sales)
```
